...
class EnvVarVisitor(ast.NodeVisitor):
    ...
    def visit_Subscript(self, node):
        # Look for os.environ["VAR"] patterns
    ...
    def visit_Assign(self, node):
        # Look for variables with default values in function parameters
        # This helps detect variables like max_retries=3 in function definitions
        if isinstance(node.value, ast.Constant):
            if isinstance(node.targets[0], ast.Name):
                var_name = node.targets[0].id
                # Look for common environment variable patterns
                if var_name.isupper() and ("_MAX_" in var_name or "_NAME" in var_name):
                    self.env_vars[var_name] = str(node.value.value)
